...
func day8_1(input string) {
	...
	for row, _ := range grid {
		for col, _ := range grid[row] {
			if grid[row][col] != '.' {
				...
				// like leetcode to find numbers of required sum
			}
		}
	}
	...
	for _, positions := range antennas {
		for i := 0; i < len(positions); i++ {
			for j := i + 1; j < len(positions); j++ {
				pos1 := positions[i]
				pos2 := positions[j]
				// Get locations of two antennas, then compare

				// Cal the delta between
				deltaRow := pos2[0] - pos1[0]
				deltaCol := pos2[1] - pos1[1]

				// Cal the locations of antinodes by minus delta top, plus delta the bottom
	...
	fmt.Println("Output of task 01 day 08 is ", output)
}
...
func day8_2(input string) {
	...
	for _, positions := range antennas {
		for i := 0; i < len(positions); i++ {
			for j := i + 1; j < len(positions); j++ { // ?! why j = i +1 // to get two postions from one list (two loop or two window slides)
				pos1 := positions[i]
				pos2 := positions[j]
				...
				// Cal the delta, then keep looping subtract/add delta to check locations within the map.
	...
	output := len(antiNodes)
	fmt.Println("Output of task2 day8 is", output)
}
